...
import { getNewsItemHistory, getNewsItemHistoryOwn } from '../../../services/dashboardService';
...
export const ItemNews = (props: Props) => {
    const { t } = useTranslation();
    const { data } = useQuery({
        queryKey: ['newsItem'],
        queryFn: getNewsItemHistoryOwn,
    });

    return (
        <div className="h-full max-w-[900px]">
            <h3 className="font-bold text-[18px]">{t('recently_items')}</h3>
            <PerfectScrollbar className="max-h-[300px] p-2 pt-0">
                          ...
                          <div className="flex" key={item.id}>
                              ...
                              <div className="p-2.5 self-center ltr:ml-2.5 rtl:ltr:mr-2.5 rtl:ml-2.5">
                                  ...
                                  <p className="text-[#3b3f5c] dark:text-white-light font-semibold text-md">
                                      {item?.user?.username} {t('message_news_item')} {item?.provider}
                                  </p>
                              </div>
                          </div>
                      ))

...
import { getItemHistoryByUserId, reDownloadItem } from '../../../services/itemService';
...
export const ItemHistoryList = (props: Props) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle(t('item_download_history')));
    }, []);
    ...
    const {
        ...
        queryKey: ['itemHistory', user?.id],
        queryFn: getItemHistoryByUserId,
        enabled: !!user?.id,
        onSuccess: (data) => {
    ...
    const { mutate: reDownloadItemMutation } = useMutation({
        ...
        onSuccess: (data) => {
            ...
            setListItemId((prev) => [...prev, data?.data?.itemDCode]);
            ...
            },
            {
                ...
                render: ({ cost }) => {
                    return (
                        <>
                            {cost
                                ? (Math.ceil((cost * parseFloat(systemConfig.usd_vnd)) / 100) * 100).toLocaleString('vi-VN', {

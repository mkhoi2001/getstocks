...
import { getAllPackageG, createPackageG, deletePackageG } from '../../../services/packageService';
...
import PackageGForm from './Components/PackageGForm/PackageGForm';
...
const Dashboard = (props: Props) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle(t('package_g')));
    }, []);
    const [openConfirmModal, setOpenConfirmModal] = useState<boolean>(false);
    const [packageId, setPackageId] = useState<string>('');
    const [initialRecords, setInitialRecords] = useState<PackagePricingG[]>([]);
    ...
    const {
        mutate: createPackageGMutation,
        isLoading: isLoadingCreate,
        isSuccess,
    } = useMutation({
        mutationFn: createPackageG,
        onSuccess: (data) => {
    ...
    const { mutate: deletePackageGMutation } = useMutation({
        mutationFn: deletePackageG,
        onSuccess: (data) => {
            refetch();
    ...
    const submitFormDelete = () => {
        deletePackageGMutation(packageId);
    };
    ...
    useEffect(() => {
        setInitialRecords(() => {
            return [
                ...orderBy(
                    rowData?.data.content.filter((item: PackagePricingG) => {
                        return (
                            item?.name?.toLowerCase().includes(search.toLowerCase()) ||
                            item?.price?.toString().includes(search.toLowerCase()) ||
                            item?.balanceG?.toString().includes(search.toLowerCase()) ||
                            item?.balanceP?.toString().includes(search.toLowerCase())
                        );
                    }),
                    'updatedAt',
                    'desc'
    ...
    const columns: DataTableColumn<PackagePricingG>[] = useMemo(
        () => [
            {
                accessor: 'id',
                title: 'ID',
            ...
            {
                accessor: 'balanceG',
                title: t('g_plus_package'),
                sortable: true,
            },
            {
                accessor: 'balanceP',
                title: t('p_plus_package'),
                sortable: true,
            },
            {
                accessor: 'content',
                title: t('content_payment'),
                sortable: true,
            },
            {
                accessor: 'action',
                        ...
                        <div className="flex items-center gap-2">
                            <Tippy content={t('edit')}>
                                <button
                                    type="button"
                                    onClick={() => {
                                        navigate(`${AdminRoutes.PACKAGE_G}/${id}`);
                                    }}
                                >
                                    <svg
            ...
            {isAdded ? (
                <div className="panel">
                    ...
                    <div className="pt-5 p-2">
                        <PackageGForm isSuccess={isSuccess} isLoading={isLoadingCreate} mutation={createPackageGMutation} isNew={true} />
                    </div>
                </div>
            ) : (
                <div>
                    <CustomDataTable<PackagePricingG>
                    ...
                    {openConfirmModal ? (
                        <ConfirmModal
                            ...
                            message={t('confirm_delete_package')}
